...
```text
✅ 建立多来源网站词条爬取 Connector、检查点和 URL 级处理记录
✅ 接入波普词典，支持检查点为空时完整枚举、有检查点时增量枚举
✅ 支持网页手动爬取、定时爬取、停止任务、失败重试和中断恢复
✅ 对正式词条、正式变体和全部候选状态执行统一判重
✅ 非重复词条自动进入候选词条池，并保留来源分类、标签和例句
...
[ ] 当来源配置复杂度提高时，引入统一来源表和可视化 Connector 配置管理
